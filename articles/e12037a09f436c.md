...
AIの進歩が著しい今の時代においても、あまり要領がよくない私にとって「今は何をしている」「この差し込みタスクは後回し！」と整理をつけることは結構重要です。
...
大きめのタスクの場合、あえて`main.md`からは消さないようにしています。ですが、すでにアーカイブ用のディレクトリに移動済みであるため、不要になったらシンプルにこの行を消すだけで十分です。
...
以下のようなコードで実演したコマンドを定義しています。まだ正規表現が甘い部分がありますし、生成されるファイルの内容は私が使うのに必要十分な内容になっていますので、好きにカスタマイズしていただければと思います。
...
とはいえ、ノートの代わりにSlackのスレッドに細かい作業や悩みを書き出しておくこともあります。例えば障害対応などは個人的にメモを残している場合ではないし、ふさわしくないと思います。私は個人として集中したり、考えを深める必要があることだけに使っています。（個人的な教訓や反省を残していたりなど）

### タスク管理TIPS

ここまで紹介しているタスク管理と合わせて使うとよかった方法をまとめておきます。
...
以上、私個人のタスク管理の方法について記載しました。タスク管理に関しては、みなさん自分なりの方法を持っていると思いますので、ぜひ色々な方法を発信して見せていただきたいですね。
